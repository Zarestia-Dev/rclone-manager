...
// Total: 104 endpoints across 14 modules (77 active, 27 commented).
//
// To regenerate:
...
pub mod operations {
    ...
    /// Perform archive create, extract, or list operations cleanly in process. (librclone custom patch)
    ...
    /// Perform cryptcheck on encrypted remotes directly in process. (librclone custom patch)
    ...
    /// - maxSize - (optional) maximum size limit in bytes (default 2097152 / 2MiB)
    /// - base64 - (optional) if true, returns the file contents base64-encoded (default false)
    pub const GETFILE: &str = "operations/getfile";
