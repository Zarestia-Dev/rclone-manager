...
pub async fn verify_rclone_sha256(zip_path: &Path, version: &str, platform_zip_name: &str) -> Result<(), String> {
    let sha_url = format!("https://downloads.rclone.org/v{}/SHA256SUMS", version);

    println!("Fetching SHA256SUMS from: {}", sha_url);
    println!("Verifying file: {}", platform_zip_name);
    println!("Using zip path: {}", zip_path.display());
    println!("Using version: {}", version);
    println!("Using platform zip name: {}", platform_zip_name);
    println!("Using SHA256SUMS URL: {}", sha_url);

    let response = get(&sha_url).await.map_err(|e| format!("Failed to fetch SHA256SUMS: {e}"))?;
    let text = response.text().await.map_err(|e| format!("Failed to read SHA256SUMS: {e}"))?;

    let wanted_hash = text.lines()
