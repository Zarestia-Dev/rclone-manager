...
/// Normalize a remote name by removing trailing ':' and runtime instance suffixes
/// generated by rclone (e.g. `Mega{Gyju7}` -> `Mega`).
///
/// Returns `Cow<'_, str>` with zero heap allocations for standard remote names.
#[must_use]
pub fn normalize_remote_name(remote: &str) -> Cow<'_, str> {
    let trimmed = remote.trim().trim_end_matches(':');
    if let (Some(open), true) = (trimmed.rfind('{'), trimmed.ends_with('}'))
        && open > 0
    {
        let suffix = &trimmed[(open + 1)..(trimmed.len() - 1)];
        let is_runtime_suffix = !suffix.is_empty()
            && suffix
                .chars()
                .all(|c| c.is_ascii_alphanumeric() || c == '_' || c == '-');
        if is_runtime_suffix {
            return Cow::Borrowed(&trimmed[..open]);
        }
    }
    Cow::Borrowed(trimmed)
}

/// Extract and normalize the remote name from an rclone fs string.
///
/// Returns `Cow<'_, str>` with zero heap allocations.
/// Examples:
/// - `Mega{Gyju7}:` -> `Mega`
/// - `Mega{Gyju7}:/folder` -> `Mega`
/// - `OneDrive:` -> `OneDrive`
#[must_use]
pub fn extract_remote_name_from_fs(fs: &str) -> Cow<'_, str> {
    let remote_part = fs.split(':').next().unwrap_or_default();
...
/// Normalizes an option key by stripping leading `-` or `--` and replacing `-` with `_`.
///
/// Uses `Cow<'_, str>` to eliminate heap allocations when no hyphens need replacement.
///
/// For example:
/// - `"--value-of-rclone"` -> `"value_of_rclone"` (Owned)
/// - `"--bwlimit"` -> `"bwlimit"` (Borrowed)
/// - `"-v"` -> `"v"` (Borrowed)
/// - `"vfs-cache-mode"` -> `"vfs_cache_mode"` (Owned)
/// - `"fast_list"` -> `"fast_list"` (Borrowed)
#[must_use]
pub fn normalize_option_key(key: &str) -> Cow<'_, str> {
    let stripped = key.trim_start_matches('-');
    if stripped.contains('-') {
        Cow::Owned(stripped.replace('-', "_"))
    } else {
        Cow::Borrowed(stripped)
    }
}

/// Returns true if the key represents a flat option and is not a path key.
/// A flat option is any key whose normalized form starts with a lowercase letter or underscore.
...
#[must_use]
pub fn interpolate_shell_commands(s: &str) -> Cow<'_, str> {
...
pub fn interpolate_value(value: &Value) -> Value {
    match value {
        Value::String(s) => match interpolate_shell_commands(s) {
            Cow::Borrowed(_) => value.clone(),
            Cow::Owned(interpolated) => Value::String(interpolated),
        },
        Value::Object(map) => {
            let new_map = map
                .iter()
...
/// Zero heap allocations.
#[must_use]
pub fn normalize_windows_path(path: &str) -> &str {
    let mut p = path;
...
mod tests {
    ...
    #[test]
    fn test_normalize_remote_name_runtime_suffix() {
        assert_eq!(normalize_remote_name("Mega{Gyju7}"), "Mega");
        assert_eq!(normalize_remote_name("Mega{Gyju7}:"), "Mega");
        assert_eq!(normalize_remote_name("OneDrive"), "OneDrive");
    }

    #[test]
    fn test_extract_remote_name_from_fs_runtime_suffix() {
    ...
    #[test]
    fn test_normalize_option_key() {
        assert_eq!(normalize_option_key("--value-of-rclone"), "value_of_rclone");
        assert_eq!(normalize_option_key("--bwlimit"), "bwlimit");
        assert_eq!(normalize_option_key("-v"), "v");
        assert_eq!(normalize_option_key("--vfs-cache-mode"), "vfs_cache_mode");
        assert_eq!(normalize_option_key("vfs-cache-mode"), "vfs_cache_mode");
        assert_eq!(normalize_option_key("fast_list"), "fast_list");
        assert_eq!(normalize_option_key("CacheMode"), "CacheMode");
    }
    ...
    fn test_is_flat_option_key() {
        ...
        assert!(is_flat_option_key("--value-of-rclone"));
        assert!(is_flat_option_key("--bwlimit"));
        assert!(is_flat_option_key("-v"));
        assert!(is_flat_option_key("vfs-cache-mode"));
