...
impl WatcherManager {
    ...
    /// Sync file watchers for Quick Runs.
    pub async fn sync_quick_run_watchers(&self, app_handle: AppHandle) -> Result<(), String> {
        let backend_manager = app_handle.state::<BackendManager>();
        if !backend_manager.is_active_local().await {
            return Ok(());
        }
        ...
        let active_qr_keys: std::collections::HashSet<String> = quick_runs
            .iter()
            .filter(|qr| qr.is_watch_enabled())
            .map(|qr| format!("qr:{}", qr.id))
            .collect();

        for qr in &quick_runs {
            let session_key = format!("qr:{}", qr.id);
            if !active_qr_keys.contains(&session_key) || sessions.contains_key(&session_key) {
                continue;
            }
            match self
                .start_quick_run_watch_session(qr, app_handle.clone())
                .await
            {
                Ok(session) => {
                    sessions.insert(session_key.clone(), session);
                    log::info!("Started watcher for quick run: {}", qr.name);
                }
                Err(e) => {
                    log::warn!("Failed to start watcher for quick run {}: {e}", qr.name);
                }
            }
        }

        sessions.retain(|key, _| {
            if !key.starts_with("qr:") || active_qr_keys.contains(key) {
                return true;
            }
            log::info!("Stopped watcher for quick run session: {key}");
            false
        });
    ...
    async fn start_quick_run_watch_session(
        ...
        if valid_paths.is_empty() {
            return Err(format!(
                "No valid local paths to watch for quick run '{}'",
                quick_run.name
            ));
        }
        ...
        for p in &valid_paths {
            ...
            log::info!("Quick run watching path: {p}");
        }
        ...
        tokio::spawn(async move {
            ...
            loop {
                let timeout = tokio::time::sleep(debounce_dur);
                tokio::pin!(timeout);
                ...
                tokio::select! {
                    res = rx.recv() => {
                        match res {
                            ...
                            Some(Err(e)) => log::debug!("Watcher error for quick run {qr_id}: {e}"),
                            None => break,
                        }
                    }
