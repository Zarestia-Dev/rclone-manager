use crate::core::security::{
    ...
    test_rclone_password, EnvironmentManager, PasswordValidatorState
};
...
pub async fn remove_config_password() -> Result<(), String> {
    ...
    match credential_store.remove_config_password() {
        Ok(()) => {
            ...
            EnvironmentManager::clear_config_password_env();
...
pub async fn validate_rclone_password(
    ...
    if let Ok(validator) = password_state.lock() && validator.is_locked_out() {
        let remaining = validator.remaining_lockout_time()
            .map(|d| d.as_secs())
            .unwrap_or(0);
        
        error!("🔒 Password validation locked out for {} seconds", remaining);
        return Err(format!(
...
// /// Clear all stored credentials (for security/privacy)
// #[tauri::command]
// pub async fn clear_all_credentials(app: AppHandle) -> Result<(), String> {
//     info!("🧹 Clearing all stored credentials");
    
//     let credential_store = CredentialStore::new();
//     match credential_store.clear_all_credentials() {
//         Ok(()) => {
...
//         }
//         Err(errors) => {
//             error!("❌ Some credentials failed to clear: {:?}", errors);
//             Err(format!("Some credentials failed to clear: {:?}", errors))
//         }
//     }
// }
...
#[tauri::command]
pub async fn set_config_password_env(password: String) -> Result<(), String> {
...
#[tauri::command]
pub async fn clear_config_password_env() -> Result<(), String> {
...
#[tauri::command]
pub async fn has_config_password_env() -> Result<bool, String> {
    debug!("🔍 Checking config password environment variable");
    
    Ok(EnvironmentManager::has_config_password_env())
}
...
pub async fn is_config_encrypted(app: AppHandle) -> Result<bool, String> {
    ...
    match result {
        Ok(Ok(output)) => {
            ...
            debug!("✅ rclone config encryption check stdout: {}", stdout.trim());
        ...
        Err(_timeout) => {
            debug!("⏱️ rclone config encryption check timed out (likely waiting for password input)");
...
pub async fn encrypt_config(
    ...
    let password_command = if cfg!(windows) {
        ...
        format!("powershell -Command \"Write-Host {} -NoNewline\"", password.replace("'", "''"))
    } else {
    ...
    let output = child.wait_with_output().await.map_err(|e| format!("Failed to wait for rclone config encryption set: {e}"))?;
    ...
    if output.status.success() || stdout.contains("Password set") || stdout.contains("Your configuration is encrypted") {
        ...
        error!("❌ Failed to encrypt configuration - stdout: {}, stderr: {}", stdout, stderr);
        Err(format!("Failed to encrypt configuration: {}", if !stderr.trim().is_empty() { stderr } else { stdout }))
    }
}
...
pub async fn unencrypt_config(
    ...
    debug!("🔧 Running rclone config encryption remove with password-command");
    ...
    if output.status.success() || stdout.contains("Your configuration is not encrypted") || stderr.contains("config file is NOT encrypted") {
        // Remove stored password since config is no longer encrypted
        let credential_store = CredentialStore::new();
        if let Err(e) = credential_store.remove_config_password() {
            warn!("⚠️ Failed to remove stored password after unencryption: {:?}", e);
        }
        ...
        error!("❌ Failed to unencrypt configuration - stdout: {}, stderr: {}", stdout, stderr);
        Err(format!("Failed to unencrypt configuration: {}", if !stderr.trim().is_empty() { stderr } else { stdout }))
    }
}
...
pub async fn change_config_password(
    ...
    unencrypt_config(app.clone(), current_password).await
    ...
    encrypt_config(app.clone(), password_state.clone(), new_password.clone()).await
    ...
    EnvironmentManager::set_config_password_env(&new_password);
