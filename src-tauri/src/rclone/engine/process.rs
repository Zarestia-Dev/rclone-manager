...
impl RcApiEngine {
    ...
    pub fn kill_process(&mut self) -> Result<(), String> {
        if let Some(child) = self.process.take() {
            ...
            if self.running {
                ...
                let quit_url = EndpointHelper::build_url(&self.api_url, core::QUIT);
    ...
    pub async fn kill_all_rclone_rcd(&self) -> Result<(), String> {
        kill_all_rclone_processes(self.api_port, self.oauth_port).await
    }
}
