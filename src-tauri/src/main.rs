...
fn toggle_mount_method() -> String {
    ...
    save_config(&config);
    format!("Mount method set to: {}", if config.use_rc_api { "RC API" } else { "Rust Threads" })
}


fn ensure_rc_api_running() {
    if !tokio::runtime::Runtime::new().unwrap().block_on(is_rc_api_running()) {
        println!("Starting Rclone RC API...");
        Command::new("rclone")
            .args(&["rcd", "--rc-no-auth", "--rc-serve", "--rc-addr=localhost:5572"])
            .spawn()
