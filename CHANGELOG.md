...
- **Linux NVIDIA WebKit Environment Auto-Configuration**: Automatically configures WebKitGTK rendering environment variables (`WEBKIT_DISABLE_DMABUF_RENDERER=1`, `WEBKIT_DISABLE_COMPOSITING_MODE=1`) at startup when an NVIDIA GPU is detected on Linux desktop sessions, resolving blank windows and Wayland protocol errors while preserving native display backends. Thanks to [@nvandamme](https://github.com/nvandamme)! PR #300
