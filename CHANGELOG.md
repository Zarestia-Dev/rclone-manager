...
### Fixed
- **Linux System Sleep & Shutdown Inhibitor Deadlock**: Resolved an issue on Linux where active operations holding a `systemd-logind` power inhibitor lock prevented system reboot/shutdown (causing black-screen hangs on desktop environments like KDE Plasma 6) and prompted for administrator/Polkit passwords on manual sleep. Simplified the inhibitor lock to unprivileged idle-sleep blocking (`what = "idle", mode = "block"`), allowing passwordless manual sleep and lid-close sleep, while listening to logind `PrepareForShutdown(true)` signals to cleanly stop active jobs and unmount remotes before shutdown. Fixes #299
