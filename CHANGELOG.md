...
- Detecting the metered connection and showing a warning banner. Not supported on macOS because it does not support metered network detection (For now, it is only show the warning banner.).

### Changed
- Mount path selection not forced to select a path from the file browser. You can also type the path manually but it will be validated.
