...
export class VfsService {
  ...
  async listVfs(): Promise<VfsList> {
    try {
      return this.apiClient.invoke('vfs_list');
    } catch (error) {
      console.error('Error fetching VFS list:', error);
      throw error;
    }
  }
  ...
  async forget(fs: string, file?: string): Promise<VfsForgetResponse> {
    try {
      console.log('Forgetting VFS item:', { fs, file });
      return this.apiClient.invoke('vfs_forget', { fs, file });
    } catch (error) {
      console.error('Error forgetting VFS item:', error);
      throw error;
    }
  }
  ...
  async refresh(fs: string, dir?: string, recursive = false): Promise<void> {
    try {
      ...
      console.error('Error refreshing VFS directory:', error);
      throw error;
    }
  }
  ...
  async getQueue(fs: string): Promise<VfsQueueResponse> {
    try {
      ...
      console.error('Error fetching VFS queue:', error);
      throw error;
    }
  }

  async setPollInterval(fs: string, interval: string): Promise<any> {
    try {
      return this.apiClient.invoke('vfs_poll_interval', { fs, interval });
    } catch (error) {
      console.error('Error setting VFS poll interval:', error);
      throw error;
    }
  }
  ...
  async getPollInterval(fs: string): Promise<VfsPollIntervalResponse> {
    try {
      return this.apiClient.invoke('vfs_poll_interval', { fs });
    } catch (error) {
      console.error('Error getting VFS poll interval:', error);
      throw error;
    }
  }

  async setQueueExpiry(fs: string, id: number, expiry: number, relative: boolean): Promise<any> {
    try {
      return this.apiClient.invoke('vfs_queue_set_expiry', { fs, id, expiry, relative });
    } catch (error) {
      console.error('Error setting VFS queue expiry:', error);
      throw error;
    }
  }
}
